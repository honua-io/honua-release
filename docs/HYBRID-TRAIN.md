...
| MCP/Studio/GP-execute/top-demo extended scenarios | **Neither yet** — hardcoded `blocked` | Needs the driver toolchain packaged as a harness image (honua-release#129). Not a hybrid-lane decision; a genuine gap. |
...
| `gate_e2e` / `gate_cloud-parity` extended scenarios (MCP/Studio/GP-execute/top-demo) | Blocked on the cloud harness image, a genuine build gap, not a policy choice. | honua-release#129 |
| Cloud-tier data-dependent canary probes (render+query smoke, per-service WMS/WMTS/WCS, tile.json) against a bare terraform-provisioned cell | No seed-data provisioning story for ephemeral cloud cells yet — legitimately unprovable, reported `blocked`, not faked. | Follow-up to honua-release#61 (not yet filed as its own issue — seed-data-on-provision is a natural next increment once #129's harness image work is scoped). |
| Cloud-tier `security-headers`: the HSTS half, on a plain-HTTP parity cell | The ephemeral cells publish a raw cloud load balancer with no certificate and no DNS name to put one on, and RFC 6797 §7.2 requires a client to IGNORE `Strict-Transport-Security` received over plain HTTP — honua-server deliberately does not emit it there. The probe asserts the four transport-independent headers on every cell and HSTS only on an `https://` endpoint (the serverless cell's API Gateway, and demo.honua.io), and its pass message names the header it did not assert so a green cell never reads as "HSTS verified". | Not a harness gap: proving HSTS on these cells would need per-run certificates and DNS. The `https://` serverless cell proves it for the candidate. |
