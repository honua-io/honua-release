...
2. Bind the exact gRPC and SDK versions and protected-branch SHAs selected for
   this train in `platform-manifest.yaml`, then validate it against
   `compatibility-matrix.yaml`. Change the matrix only if the train changes an
   actual compatibility rule; its SDK ranges are support windows, not candidate
   pins. Retain the successful validation receipt before configuring
